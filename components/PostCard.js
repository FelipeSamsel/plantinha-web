...
function Avatar({ url, name, size = 36 }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: '#EAF3DE', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 500, color: '#3B6D11', fontSize: size * 0.38, flexShrink: 0 }}>{ini}</div>
  )
}
...
function CommentItem({ comment, user, onDelete }) {
  ...
  async function sendReply() {
    ...
    if (comment.user_id !== user.id) {
      ...
      await supabase.from('notifications').insert({ user_id: comment.user_id, from_user_id: user.id, type: 'comment', post_id: comment.post_id, message: `${profile?.username ?? 'Alguém'} respondeu seu comentário: "${newReply.trim().slice(0, 50)}${newReply.length > 50 ? '...' : ''}"` })
    }
    ...
    <div style={{ display: 'flex', gap: 10, padding: '10px 16px', alignItems: 'flex-start' }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 2 }}>
          ...
          {isOwner && <button onClick={() => onDelete(comment.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#B4B2A9', fontSize: 11, padding: 0 }} onMouseOver={e => e.target.style.color = '#993C1D'} onMouseOut={e => e.target.style.color = '#B4B2A9'}>excluir</button>}
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          {replyCount > 0 && <button onClick={() => setShowReplies(!showReplies)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: '#3B6D11', padding: 0 }}>{showReplies ? 'ocultar' : `ver ${replyCount} resposta${replyCount > 1 ? 's' : ''}`}</button>}
        </div>
        {replying && (
          <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
            <input value={newReply} onChange={e => setNewReply(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendReply()} placeholder={`Responder ${comment.profiles?.username ?? ''}...`} autoFocus style={{ flex: 1, border: '0.5px solid #C5E4A7', borderRadius: 20, padding: '7px 12px', fontSize: 12, outline: 'none', fontFamily: 'inherit', background: '#F4FAF0' }} />
            <button onClick={sendReply} disabled={loadingReply || !newReply.trim()} style={{ background: newReply.trim() ? '#3B6D11' : '#C5E4A7', border: 'none', borderRadius: 20, padding: '7px 12px', fontSize: 12, fontWeight: 500, color: '#EAF3DE', cursor: newReply.trim() ? 'pointer' : 'default', fontFamily: 'inherit' }}>{loadingReply ? '...' : '↩'}</button>
              ...
              <div key={reply.id} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 2 }}>
                    ...
                    {user && user.id === reply.user_id && <button onClick={() => deleteReply(reply.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#B4B2A9', fontSize: 10, padding: 0 }} onMouseOver={e => e.target.style.color = '#993C1D'} onMouseOut={e => e.target.style.color = '#B4B2A9'}>excluir</button>}
                  </div>
